"""
AdaBoost Classification and Regression Router for FastAPI
Adaptive boosting over shallow decision trees ("stumps")
"""
...
class AdaBoostRequest(BaseModel):
    ...
    base_max_depth: int = 1  # depth of the base "stump" — 1 is classic AdaBoost
    random_state: int = 42
    cv_folds: int = 5
...
def generate_interpretation(result: Dict, task_type: str, feature_importance: List[Dict], params: dict) -> Dict[str, Any]:
    ...
    # Staged performance — did the ensemble converge?
...
async def run_adaboost_analysis(request: AdaBoostRequest) -> Dict[str, Any]:
    """
    Train AdaBoost for classification or regression.

    Supports:
    - Binary and multi-class classification
    - Regression
    - Staged (per-boosting-round) performance tracking
    """
    try:
        data = request.data
        ...
        params = {
            ...
            'base_max_depth': request.base_max_depth,
        ...
        perm_importance = compute_permutation_importance(model, X_test.values if hasattr(X_test, 'values') else X_test, y_test_for_perm, feature_cols)
        ...
        raise HTTPException(status_code=500, detail=f"AdaBoost analysis failed: {str(e)}")
