"""
CatBoost Classification and Regression Router for FastAPI
Ordered boosting with native categorical feature support (no manual encoding needed)
"""
...
async def run_catboost_analysis(request: CatBoostRequest) -> Dict[str, Any]:
    """
    Train CatBoost model for classification or regression.

    Supports:
    - Binary and multi-class classification
    - Regression
    - Native categorical feature handling (no manual one-hot encoding)
    - Early stopping with learning-curve tracking
    """
    try:
        data = request.data
        ...
        raise HTTPException(status_code=500, detail=f"CatBoost analysis failed: {str(e)}")
