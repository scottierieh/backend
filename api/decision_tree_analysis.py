"""
Decision Tree Classification and Regression Router for FastAPI
Includes: Normalized Importance, SHAP, Partial Dependence Plot, Tree Rule Extraction
"""
...
async def run_decision_tree(request: DecisionTreeRequest) -> Dict[str, Any]:
    """
    Decision Tree for classification and regression.
    Returns: metrics, normalized feature importance, SHAP,
             partial dependence plots, tree rules, visualizations.
    """
    try:
        if not request.data:
        ...
        if missing:
            raise HTTPException(status_code=400,
                                detail=f"Columns not found: {', '.join(missing)}")
        ...
        if len(X) < 50:
            raise HTTPException(status_code=400,
                                detail="At least 50 valid samples required.")
        ...
        criterion = _fix_criterion(request.criterion, task_type)
        ...
        params = {
            ...
            'max_features':      (request.max_features
                                  if request.max_features not in (None, 'None')
                                  else None),
            'criterion':         criterion,
        ...
        # ── CV ──
        cv_result = perform_cv(X_arr, y, params, task_type, request.cv_folds)
        ...
        raise HTTPException(status_code=500,
                            detail=f"Decision Tree analysis failed: {str(e)}")
