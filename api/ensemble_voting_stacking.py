"""
Voting & Stacking Ensemble Router for FastAPI
...
class EnsembleRequest(BaseModel):
    ...
    voting_type: str = "soft"  # hard, soft (classification only)
    base_estimators: Optional[List[str]] = None  # subset of: logistic_regression/ridge, decision_tree, random_forest, svm, knn
    final_estimator: str = "logistic_regression"  # meta-learner for stacking
    cv_folds: int = 5
    random_state: int = 42
...
def perform_cross_validation(X, y, task_type: str, params: dict, cv_folds: int) -> Dict[str, Any]:
    ...
    return {
        'cv_scores': _to_native_scalar_list(scores), 'cv_mean': _to_native_type(np.mean(scores)),
        'cv_std': _to_native_type(np.std(scores)), 'cv_folds': cv_folds
    }
...
@router.post("/ensemble-voting-stacking")
async def run_ensemble_analysis(request: EnsembleRequest) -> Dict[str, Any]:
    """
    Train a Voting or Stacking ensemble for classification or regression,
    combining multiple base learners (logistic/ridge, decision tree, random forest, SVM, KNN).
    """
    try:
        data = request.data
        ...
        if request.ensemble_method not in ('voting', 'stacking'):
            raise HTTPException(status_code=400, detail="ensemble_method must be 'voting' or 'stacking'.")
        ...
        params = {
            ...
            'base_estimators': request.base_estimators,
