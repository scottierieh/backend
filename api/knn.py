"""
K-Nearest Neighbors (KNN) Classification and Regression Router for FastAPI
...
class KNNRequest(BaseModel):
    ...
    metric: str = "minkowski"  # euclidean, manhattan, minkowski, chebyshev, cosine, hamming
    p: int = 2  # Power parameter for Minkowski (1=manhattan, 2=euclidean)
    algorithm: str = "auto"  # auto, ball_tree, kd_tree, brute
    random_state: int = 42
    cv_folds: int = 5
    scale_features: bool = True
    find_optimal_k: bool = True
    k_range: List[int] = [1, 3, 5, 7, 9, 11, 13, 15]

    VALID_METRICS: ClassVar[set] = {'euclidean', 'manhattan', 'minkowski', 'chebyshev', 'cosine', 'hamming'}

    def validate_inputs(self):
        """Validate request parameters, raise ValueError on invalid input."""
        if self.metric not in self.VALID_METRICS:
            raise ValueError(
                f"Invalid metric '{self.metric}'. "
                f"Supported: {sorted(self.VALID_METRICS)}"
            )
        cleaned = [k for k in self.k_range if isinstance(k, int) and 1 <= k <= 500]
        if not cleaned:
            raise ValueError("k_range must contain at least one integer in [1, 500].")
        self.k_range = sorted(set(cleaned))
...
@router.post("/knn")
async def run_knn_analysis(request: KNNRequest) -> Dict[str, Any]:
    """
    Train K-Nearest Neighbors model for classification or regression.
    
    Supports:
    - Binary and multi-class classification
    - Regression
    - Automatic K selection via cross-validation
    ...
    try:
        ...
        if len(X) < 30:
            raise HTTPException(status_code=400, detail="At least 30 valid samples required.")
        ...
        scaler = None
        X_array = X.values
        if request.scale_features:
            scaler = StandardScaler()
            X_array = scaler.fit_transform(X_array)

        # ── Split data ──────────────────────────────────────────────
        X_train, X_test, y_train, y_test = train_test_split(
            X_array, y, test_size=request.test_size, random_state=request.random_state,
            stratify=y if task_type == 'classification' else None
        )

        # ── Clamp n_neighbors to valid range ────────────────────────
        max_k = len(X_train) - 1
        n_neighbors = min(max(1, request.n_neighbors), max_k)

        # ── Find optimal K (pass cv_folds) ──────────────────────────
        k_search_result = None
        if request.find_optimal_k:
            k_search_result = find_optimal_k(
                X_train,
                y_train if task_type == 'regression' else LabelEncoder().fit_transform(y_train),
                request.k_range, task_type, request.weights, request.metric, request.p,
                cv_folds=request.cv_folds   # ← was hardcoded to 5
            )
        ...
        raise HTTPException(status_code=500, detail=f"KNN analysis failed: {str(e)}")
