"""
LightGBM Classification and Regression Router for FastAPI
Leaf-wise gradient boosting with native categorical support
"""
...
class LightGBMRequest(BaseModel):
    ...
    max_depth: int = -1  # -1 = no limit (leaf-wise growth controlled by num_leaves)
    num_leaves: int = 31
...
async def run_lightgbm_analysis(request: LightGBMRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        params = {
            ...
            'min_child_samples': request.min_child_samples,
        ...
        raise HTTPException(status_code=500, detail=f"LightGBM analysis failed: {str(e)}")
