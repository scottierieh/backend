"""
Artificial Neural Network (Multi-Layer Perceptron) Router for FastAPI
Standalone MLP classifier/regressor with loss-curve tracking
"""
...
@router.post("/mlp")
async def run_mlp_analysis(request: MLPRequest) -> Dict[str, Any]:
    """
    Train a Multi-Layer Perceptron (Artificial Neural Network) for classification or regression.
    """
    try:
        data = request.data
        ...
        if not request.hidden_layer_sizes:
            raise HTTPException(status_code=400, detail="hidden_layer_sizes must be a non-empty list.")

        if task_type == 'auto':
            task_type = detect_task_type(y)

        params = {
            'hidden_layer_sizes': request.hidden_layer_sizes,
        ...
        raise HTTPException(status_code=500, detail=f"MLP analysis failed: {str(e)}")
