"""
Naive Bayes Classification Router for FastAPI
...
class NaiveBayesRequest(BaseModel):
    ...
    nb_type: str = "gaussian"  # gaussian, multinomial, bernoulli
    var_smoothing: float = 1e-9  # for GaussianNB
    alpha: float = 1.0  # for MultinomialNB and BernoulliNB (Laplace smoothing)
    fit_prior: bool = True
    binarize_threshold: Optional[float] = None  # for BernoulliNB: threshold for binarization (None = use 0.0)
    random_state: int = 42
    cv_folds: int = 5

    VALID_NB_TYPES: ClassVar[set] = {'gaussian', 'multinomial', 'bernoulli'}

    def validate_inputs(self):
        if self.nb_type not in self.VALID_NB_TYPES:
            raise ValueError(
                f"Invalid nb_type '{self.nb_type}'. "
                f"Supported: {sorted(self.VALID_NB_TYPES)}"
            )
        if self.cv_folds < 2:
...
@router.post("/naive-bayes")
async def run_naive_bayes_analysis(request: NaiveBayesRequest) -> Dict[str, Any]:
    """
    ...
    - Feature importance (based on variance/probability)
    - Cross-validation
    """
    try:
        data = request.data
        ...
        # ── Parameters ──────────────────────────────────────────────
        params = {
            'nb_type':             request.nb_type,
            'var_smoothing':       request.var_smoothing,
            'alpha':               request.alpha,
            'fit_prior':           request.fit_prior,
            'binarize_threshold':  request.binarize_threshold,
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X_array, y, test_size=request.test_size,
            random_state=request.random_state, stratify=y
        )
        ...
        # ── Feature importance ──────────────────────────────────────
        feature_importance = get_feature_importance_nb(model, feature_cols, request.nb_type)

        # ── Cross-validation ────────────────────────────────────────
        cv_result = perform_cross_validation(X_array, y, params, request.cv_folds)
        ...
        prob_dist_plot   = generate_probability_distribution_plot(
            model, feature_cols, request.nb_type, result['class_labels']
        )
        ...
        raise HTTPException(status_code=500, detail=f"Naive Bayes analysis failed: {str(e)}")
