"""
Random Forest Classification and Regression Router for FastAPI
...
def compute_shap(model, X_train: np.ndarray, X_test: np.ndarray,
    ...
    try:
        ...
        # RF returns (n_samples, n_features, n_classes) or list per class
        sv = np.array(shap_values)
        if sv.ndim == 3:
            # shape: (samples, features, classes) → mean over classes
...
def extract_tree_rules(model, feature_names: List[str], task_type: str,
    ...
    try:
        ...
        # Leaf rules via tree structure
        tree_ = sample_tree.tree_
        feature = tree_.feature
        threshold = tree_.threshold
        n_leaves = tree_.n_node_samples
        ...
        try:
            ...
            pass  # Tree too deep — still return text rules

        # Sort by sample count, truncate
...
def generate_tree_count_plot(model, X_test, y_test, task_type: str) -> str:
    ...
    for n in tree_counts:
        if task_type == 'classification':
            # Aggregate predictions from first n trees
...
async def run_random_forest_analysis(request: RandomForestRequest) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if task_type == 'classification':
            ...
            # Encode y_test for tree count plot
        ...
        # Feature importance (Gini — unified format)
        ...
        raise HTTPException(status_code=500, detail=f"Random Forest analysis failed: {str(e)}")
