"""
Support Vector Machine (SVM) Classification and Regression Router for FastAPI
Linear and non-linear classification/regression with kernel methods
"""
...
class SVMRequest(BaseModel):
    ...
    C: float = 1.0  # Regularization parameter (log scale recommended: 0.01, 0.1, 1, 10, 100)
    gamma: str = "scale"  # Kernel coefficient: 'scale', 'auto', or numeric string e.g. '0.1'
    degree: int = 3  # Degree for poly kernel
    coef0: float = 0.0  # Independent term in kernel
    epsilon: float = 0.1  # Epsilon for SVR
    random_state: int = 42
    cv_folds: int = 5
...
@router.post("/svm")
async def run_svm_analysis(request: SVMRequest) -> Dict[str, Any]:
    """
    Train Support Vector Machine for classification or regression.
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"SVM analysis failed: {str(e)}")
