"""
Cross-Validation Analysis — CLI script
Comprehensive k-fold cross-validation for model evaluation. Ported from
scottierieh/backend's api/cross_validation.py (FastAPI router) to the
...
def main():
    try:
        ...
        scoring = payload.get('scoring', 'auto')
        ...
        # Get model
        model, model_name = get_model(model_type, task_type, random_state)
        ...
        parameters = {
            'cv_method': cv_method,
            'n_folds': n_folds,
            'n_repeats': n_repeats if cv_method in ['repeated_kfold', 'repeated_stratified'] else None,
            'shuffle': shuffle,
