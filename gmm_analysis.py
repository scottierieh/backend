"""
Gaussian Mixture Model (GMM) Clustering — CLI script
Soft/probabilistic clustering via sklearn.mixture.GaussianMixture, with BIC/AIC-based
k selection, per-component membership probabilities, and ANOVA feature drivers.
Ported from a standalone FastAPI router (api/gmm.py style) to the stdin/stdout CLI
contract used by src/backend/main.py's generic script runner — same contract as
kmeans_analysis.py / dbscan_analysis.py / hca_analysis.py.
"""
...
def main():
    try:
        ...
        n_components = int(payload.get('nComponents', 3))
        covariance_type = payload.get('covarianceType') or 'full'
        boundary_type = payload.get('boundaryType') or 'ellipse'
