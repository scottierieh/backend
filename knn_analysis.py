...
def _perm_importance_rows(perm_result, feature_names: List[str]) -> List[Dict[str, Any]]:
    """KNN has no split-based importance the way a tree does -- permutation
    ...
    if n_classes == 2:
        ...
        for i, cls in enumerate(le.classes_):
            ...
            pr_data[str(cls)] = {
                ...
                'recall': [_to_native_type(x) for x in recall],
                'ap': _to_native_type(ap),
                'base_rate': float(np.mean(y_binary == 1))
            }
    ...
    for (label, data), color in zip(pr_data.items(), colors):
        ...
        if is_binary:
            ax.axhline(y=data['base_rate'], color=color, linestyle='--', linewidth=1,
                       alpha=0.6, label=f'Baseline (positive rate = {data["base_rate"]:.3f})')
        else:
            ax.axhline(y=data['base_rate'], color=color, linestyle=':', linewidth=1, alpha=0.35)
...
def generate_regression_plots(y_test, y_pred) -> List[Dict[str, str]]:
    """Generate actual-vs-predicted and residual plots as separate single-panel images."""
    plots = []
...
def main():
    try:
        ...
        if task_type == 'classification':
            ...
            response['plots'] = regression_plots
